...
const DailyReportWizard: React.FC<DailyReportWizardProps> = ({ onClose }) => {
  ...
  const progress = reportTemplate.length > 0 ? ((stepIndex) / (reportTemplate.length)) * 100 : 100;
  ...
  const handleFinish = async (runAi: boolean = false) => {
    ...
    // Синхронізація рефлексій по звичках, якщо вони були введені, але не відмічені кнопками
    const habitQuestion = reportTemplate.find(q => q.type === 'habits');
    if (habitQuestion) {
      const habitAns = answers[habitQuestion.id];
      if (habitAns?.habitReflections) {
        Object.entries(habitAns.habitReflections).forEach(([hId, note]) => {
          if (note) {
            const h = tasks.find(t => t.id === hId);
            const currentStatus = h?.habitHistory?.[todayDateStr]?.status;
            if (!currentStatus || currentStatus === 'none') {
              toggleHabitStatus(hId, todayDateStr, 'skipped', note as string);
            }
          }
        });
      }
    ...
    if (isLastStep || !currentQuestion) {
      ...
      return (
        <div className="flex flex-col items-center justify-center py-8 text-center space-y-8 animate-in zoom-in-95 duration-500">
          <div className="w-24 h-24 rounded-[4px] bg-[var(--primary)]/10 flex items-center justify-center text-[var(--primary)] text-5xl mb-2 shadow-xl ring-4 ring-[var(--primary)]/5 relative overflow-hidden">
            <i className="fa-solid fa-sparkles text-xl absolute top-3 right-3 opacity-40"></i>
            <i className="fa-solid fa-flag-checkered"></i>
          </div>
          <div>
            <Typography variant="h3" className="text-2xl font-black uppercase tracking-tight mb-2 text-[var(--text-main)]">
              {hasEntryToday && answers.id === undefined ? 'Звіт за сьогодні вже є' : 'Підсумок дня створено'}
            </Typography>
            <p className="text-[10px] text-[var(--text-muted)] font-bold uppercase tracking-widest leading-loose">
              {hasEntryToday && answers.id === undefined
                ? 'Ви вже зафіксували свої успіхи сьогодні. Можете переглянути аналіз або оновити запис.'
                : 'Ви успішно зафіксували свій досвід.'}
              <br />
          ...
          <Card padding="md" className="border-[var(--border-color)] bg-[var(--primary)]/5 w-full max-w-xs rounded-[4px]">
            ...
            <button
              ...
              className="w-full py-4 bg-[var(--primary)] text-white rounded-[4px] text-[10px] font-black uppercase tracking-[0.2em] flex items-center justify-center gap-3 shadow-xl active:scale-95 transition-all disabled:opacity-50"
            >
              ...
              <div className="flex justify-between gap-3">
                {[1, 2, 3, 4, 5].map(i => (
                  <button
                    ...
                    className={`flex-1 py-4 rounded-[4px] border-2 transition-all text-2xl shadow-sm ${moodVal === i ? 'bg-[var(--primary)] border-[var(--primary)] text-white scale-110 shadow-lg' : 'bg-[var(--bg-card)] border-[var(--border-color)] grayscale opacity-40 hover:opacity-100'}`}
                  >
                    {moodIcons[i - 1]}
                  </button>
                ))}
          ...
          <div className="space-y-6 animate-in slide-in-from-right-4 duration-300 relative">
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-[var(--bg-input)] border-2 border-[var(--border-color)] rounded-[4px] py-3.5 pl-11 pr-5 text-sm font-bold outline-none focus:border-[var(--primary)]/30 transition-all shadow-inner text-[var(--text-main)]"
              />
              {personSearch && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-[var(--bg-card)] border border-[var(--border-color)] shadow-2xl rounded-[1.5rem] z-[3100] overflow-hidden animate-in fade-in slide-in-from-top-2 zoom-in-95 max-h-60 overflow-y-auto">
                  <div className="px-5 py-2.5 border-b border-[var(--border-color)] bg-black/[0.02]">
                  ...
                  {filteredSearchPeople.map(p => {
                    ...
                    return (
                      ...
                      >
                        <img src={p.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${p.name}`} className="w-8 h-8 rounded-[4px] object-cover border border-[var(--border-color)]" />
                  ...
                  >
                    <div className="w-8 h-8 rounded-[4px] bg-[var(--primary)] text-white flex items-center justify-center shadow-md"><i className="fa-solid fa-user-plus text-xs"></i></div>
                    <div className="flex-1 min-w-0">
            ...
            <div className="space-y-3">
              {selPeople.map((pid: string) => {
                ...
                if (!p) return null;
                return (
                  <div key={pid} className="p-4 bg-[var(--bg-card)] rounded-[4px] border-2 border-[var(--border-color)] shadow-sm space-y-3 animate-in slide-in-from-bottom-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <img src={p.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${p.name}`} className="w-7 h-7 rounded-[4px]" />
                    ...
                    <input
                      ...
                      className="w-full bg-[var(--bg-input)] border-none rounded-[4px] px-4 py-2.5 text-[11px] font-bold outline-none italic text-[var(--text-main)]"
                    />
                  </div>
                );
              })}
          ...
          <div className="space-y-4 animate-in slide-in-from-right-4 duration-300">
            <div className="grid grid-cols-1 gap-2 max-h-[240px] overflow-y-auto no-scrollbar pr-1">
              {completedToday.length > 0 ? completedToday.map(t => (
                <button
                  ...
                  className={`w-full p-5 rounded-[4px] border-2 text-left flex items-center justify-between transition-all ${vicId === t.id ? 'border-[var(--primary)] bg-[var(--primary)]/5 shadow-md' : 'bg-[var(--bg-card)] border-[var(--border-color)] hover:border-[var(--primary)]/20'}`}
                >
            ...
            <div className="space-y-2 pt-2">
              ...
              <input
                ...
                className="w-full bg-[var(--bg-input)] border-2 border-[var(--border-color)] rounded-[4px] py-4 px-5 text-sm font-bold outline-none focus:border-[var(--primary)]/30 shadow-inner text-[var(--text-main)]"
              />
            </div>
          </div>
        );
        ...
        return (
          <div className="space-y-4 animate-in slide-in-from-right-4 duration-300">
            <textarea
              ...
              className="w-full bg-[var(--bg-input)] border-2 border-[var(--border-color)] rounded-[4px] p-8 text-sm font-medium min-h-[250px] outline-none focus:border-[var(--primary)]/30 transition-all shadow-inner leading-relaxed text-[var(--text-main)]"
  ...
  return (
    <div className="fixed inset-0 z-[3000] flex items-center justify-center p-0 md:p-4 bg-black/60 backdrop-blur-xl animate-in fade-in">
      <div className="w-full max-w-lg h-full md:h-auto md:max-h-[90vh] bg-[var(--bg-card)] md:rounded-[4px] shadow-2xl flex flex-col overflow-hidden relative border border-[var(--border-color)]">
        ...
        <header className="px-8 pt-10 pb-4 flex justify-between items-start shrink-0">
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-2">
              <Badge variant="orange" className="px-2.5 py-0.5 rounded-[4px] text-[9px] font-black tracking-[0.2em]">{isLastStep ? 'ФІНАЛ' : `КРОК ${stepIndex + 1}`}</Badge>
            ...
            <Typography variant="h2" className="text-xl md:text-2xl leading-tight font-black uppercase tracking-tighter text-[var(--text-main)] pr-6">
              {isLastStep ? 'Завершення звіту' : (currentQuestion?.text || 'Питання')}
            </Typography>
          </div>
          <button onClick={onClose} className="w-12 h-12 rounded-[4px] bg-black/5 flex items-center justify-center text-[var(--text-muted)] hover:text-rose-500 transition-all shrink-0"><i className="fa-solid fa-xmark text-xl"></i></button>
        </header>

        <div className="flex-1 overflow-y-auto custom-scrollbar px-6 md:px-8 py-2 mb-2 no-scrollbar">{renderStep()}</div>

        <footer className="p-5 pb-8 md:p-10 bg-black/5 border-t border-[var(--border-color)] flex gap-3 shrink-0 no-print">
          {!isLastStep ? (
            <>
              <button
                onClick={() => stepIndex > 0 && setStepIndex(stepIndex - 1)}
                disabled={stepIndex === 0}
                className="flex-1 h-14 md:h-16 rounded-[4px] font-black uppercase text-[var(--text-muted)] text-[11px] tracking-[0.2em] hover:bg-black/5 transition-all disabled:opacity-0"
              >
                Назад
              </button>
              <Button
                variant="primary"
                disabled={!canGoNext}
                className="flex-[2] h-14 md:h-16 shadow-xl uppercase font-black tracking-[0.2em] text-[11px] rounded-[4px] group"
            ...
            <>
              <button onClick={() => setStepIndex(stepIndex - 1)} className="flex-1 h-14 md:h-16 rounded-[4px] font-black uppercase text-[var(--text-muted)] text-[11px] tracking-[0.2em] hover:bg-black/5 transition-all">Назад</button>
              <Button variant="primary" className="flex-[2] h-14 md:h-16 uppercase font-black tracking-[0.2em] text-[11px] rounded-[4px] shadow-lg" onClick={() => handleFinish(false)}>ЗБЕРЕГТИ ЗВІТ</Button>
            </>
          )}
        </footer>
      </div>
