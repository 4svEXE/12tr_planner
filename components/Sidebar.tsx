  ...
  return (
    <div className={`${isSidebarCollapsed ? 'w-16' : 'w-56'} bg-white border-r border-slate-200 flex flex-col h-screen sticky top-0 z-40 transition-all duration-300 ease-in-out`}>
      ...
      <div className="px-2 py-4 border-t border-slate-100 space-y-0.5">
        ...
        {!isSidebarCollapsed && <div className="px-3 mb-2"><span className="text-[8px] uppercase font-black tracking-widest text-slate-300">Архів</span></div>}
      ...
      {showSettings && <SettingsModal onHide={() => setShowSettings(false)} />}
    </div>
  );
};

const SettingsModal: React.FC<{ onHide: () => void }> = ({ onHide }) => {
    const { theme, setTheme, aiEnabled, setAiEnabled } = useApp();
    return (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 tiktok-blur">
          <div className="absolute inset-0 bg-slate-900/20" onClick={onHide}></div>
          <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl p-8 relative animate-in zoom-in-95 duration-200">
            <Typography variant="h2" className="text-xl mb-8">Налаштування</Typography>
            <div className="space-y-8">
               <div className="p-5 bg-orange-50/50 rounded-3xl border border-orange-100/50 flex items-center justify-between">
                  <span className="text-xs font-black uppercase">AI Асистент</span>
                  <button onClick={() => setAiEnabled(!aiEnabled)} className={`w-12 h-6 rounded-full transition-all relative ${aiEnabled ? 'bg-orange-500' : 'bg-slate-200'}`}>
                    <div className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow-sm transition-all ${aiEnabled ? 'right-1' : 'left-1'}`}></div>
                  </button>
               </div>
               <Button onClick={onHide} className="w-full py-4 rounded-3xl text-xs tracking-widest">ЗБЕРЕГТИ</Button>
            </div>
          </div>
        </div>
    );
