  ...
  const handleGlobalDrop = (e: React.DragEvent, targetId: string) => {
    ...
    switch (targetId) {
      ...
      case 'inbox': 
        ...
        break;
      case 'next_actions': 
        updateTask({ ...task, status: TaskStatus.NEXT_ACTION, category: 'tasks', isDeleted: false }); 
        break;
      case 'notes': 
    ...
    <>
      <div className={`${isSidebarCollapsed ? 'w-14' : 'w-48'} hidden md:flex bg-white border-r border-slate-100 flex-col h-screen sticky top-0 z-40 transition-all duration-300 ease-in-out shrink-0`}>
        ...
        <div className="px-1 py-4 border-t border-slate-50 space-y-0.5">
          ...
          <button onClick={() => setActiveTab('settings')} className={`w-full flex items-center gap-2 px-2.5 py-1.5 rounded-xl transition-all ${activeTab === 'settings' ? 'bg-indigo-50 text-indigo-700' : 'text-slate-400 hover:bg-slate-50'}`}>
           ...
           <div className="flex-1 overflow-y-auto p-6 bg-slate-50/30">
              <div className="grid grid-cols-3 gap-4 pb-20">
                 ...
                 <button 
                  ...
                  className={`flex flex-col items-center justify-center p-4 rounded-3xl border transition-all ${activeTab === 'settings' ? 'bg-indigo-600 text-white' : 'bg-indigo-50 border-indigo-100 text-indigo-600 shadow-sm'}`}
                 >
                   <div className={`w-10 h-10 rounded-2xl flex items-center justify-center mb-2 ${activeTab === 'settings' ? 'bg-white/10' : 'bg-white'}`}>
