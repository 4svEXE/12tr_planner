...
import { isTaskOnDay } from './MonthView';
...
export const DaySchedule: React.FC<{ currentDate: Date; onBack?: () => void; isMobile?: boolean }> = ({ currentDate, onBack, isMobile }) => {
  ...
  useEffect(() => {
    if (scheduleScrollRef.current) {
      // Scroll to 5:00 AM by default to fit 5 to 23 on screen
      const scrollPos = (5 * HOUR_HEIGHT) - 10;
      scheduleScrollRef.current.scrollTo({ top: Math.max(0, scrollPos), behavior: 'smooth' });
    }
  }, [currentDate]);
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (block) {
      ...
      updateTimeBlock({ ...block, startHour: newStartHour, endHour: newEndHour });
    }
  };
    ...
    <div className="flex flex-col h-full bg-[var(--bg-card)]">
      <header className="p-4 md:p-6 border-b border-[var(--border-color)] flex items-center justify-between shrink-0 bg-[var(--bg-card)]/80 backdrop-blur z-20">
        <div className="flex items-center gap-3">
          {isMobile && onBack && (
            <button onClick={onBack} className="w-9 h-9 rounded-xl bg-[var(--bg-main)] flex items-center justify-center text-[var(--text-muted)]"><i className="fa-solid fa-chevron-left"></i></button>
          )}
        ...
        <button onClick={() => setShowBlockModal({ dayOfWeek })} className="w-9 h-9 rounded-xl bg-[var(--text-main)] text-[var(--bg-main)] hover:opacity-90 shadow-lg transition-all flex items-center justify-center"><i className="fa-solid fa-plus text-xs"></i></button>
      </header>

      <div ref={scheduleScrollRef} className="flex-1 overflow-y-auto custom-scrollbar relative bg-[var(--bg-main)]/10 scroll-smooth">
        <div className="absolute left-0 top-0 w-full" style={{ height: 24 * HOUR_HEIGHT }} onDragOver={(e) => e.preventDefault()} onDrop={handleDrop}>
          ...
          {relevantBlocks.map(block => (
            <div key={block.id} draggable onDragStart={(e) => handleDragStart(e, block.id)} onDragEnd={handleDragEnd} onClick={() => setShowBlockModal(block)} className={`absolute left-14 right-4 rounded-2xl p-3 text-white text-[11px] font-black shadow-xl shadow-black/5 cursor-grab active:cursor-grabbing hover:brightness-110 transition-all group overflow-hidden border-2 border-white/10 ${draggedBlockId === block.id ? 'z-50' : 'z-10'}`} style={{ top: block.startHour * HOUR_HEIGHT, height: (block.endHour - block.startHour) * HOUR_HEIGHT - 4, backgroundColor: block.color || 'var(--primary)' }}>
          ...
          {isToday && (
            <div className="absolute left-0 right-0 z-30 pointer-events-none flex items-center" style={{ top: timeIndicatorPos }}>
              <div className="w-12 text-[8px] font-black text-rose-500 bg-[var(--bg-card)]/90 backdrop-blur rounded-r-lg py-0.5 px-1 shadow-sm border border-rose-100 border-l-0 text-right tabular-nums">{currentTime.toLocaleTimeString('uk-UA', { hour: '2-digit', minute: '2-digit' })}</div>
              <div className="flex-1 h-0.5 bg-rose-500/40 relative"><div className="absolute left-0 top-1/2 -translate-y-1/2 w-2.5 h-2.5 rounded-full bg-rose-500 shadow-lg ring-4 ring-rose-500/20"></div></div>
            </div>
          )}
        </div>
      </div>
      {showBlockModal && (
        <TimeBlockModal block={showBlockModal} onClose={() => setShowBlockModal(null)} onSave={(data) => { if (showBlockModal.id) updateTimeBlock({ ...data, id: showBlockModal.id } as TimeBlock); else addTimeBlock({ ...data, dayOfWeek } as Omit<TimeBlock, 'id'>); setShowBlockModal(null); }} />
      )}
      {showPresets && (
        <div className="fixed inset-0 z-[310] flex items-center justify-center p-4 tiktok-blur">
          ...
          <div className="bg-[var(--bg-card)] w-full max-w-xs rounded-[2rem] p-6 md:p-8 shadow-2xl relative animate-in slide-in-from-bottom-4 duration-300 border border-[var(--border-color)]">
            ...
            <div className="space-y-2">{routinePresets.map(p => (<button key={p.id} onClick={() => { applyRoutinePreset(p.id, dayOfWeek); setShowPresets(false); }} className="w-full p-4 rounded-2xl border border-[var(--border-color)] hover:border-[var(--primary)]/40 hover:bg-[var(--primary)]/5 text-left transition-all group"><div className="text-xs font-black text-[var(--text-main)] group-hover:text-[var(--primary)] mb-1 uppercase tracking-tight">{p.name}</div><div className="text-[8px] font-bold text-[var(--text-muted)] uppercase tracking-widest opacity-60">{p.blocks.length} блоків</div></button>))}</div>
...
const DayView: React.FC<DayViewProps> = ({ currentDate, onSelectTask, onShowScheduleMobile }) => {
  ...
  const dayTs = new Date(currentDate).setHours(0, 0, 0, 0);
  const dayTasks = tasks.filter(t => isTaskOnDay(t, dayTs));
  ...
  return (
    <div className="h-full flex flex-col gap-4 overflow-hidden relative">
          ...
          <div className="p-8 space-y-4">
            {personEvents.map(pe => (
              <div key={pe.id} className="p-5 rounded-3xl border-2 bg-amber-500/5 border-amber-500/20 flex items-center gap-4 animate-in fade-in">
                <div className="w-10 h-10 rounded-xl bg-amber-500 flex items-center justify-center text-white shadow-lg shrink-0"><i className="fa-solid fa-cake-candles"></i></div>
            ...
            <div className="space-y-3">
              {dayTasks.map(t => (
                <div key={t.id} onClick={() => onSelectTask(t.id)} className={`flex items-center gap-5 p-6 rounded-[2.5rem] transition-all cursor-pointer group border ${t.isEvent ? 'bg-[var(--primary)]/5 border-[var(--primary)]/20' : 'bg-[var(--bg-main)]/30 border-[var(--border-color)] hover:border-[var(--primary)]/40 shadow-sm'}`}>
                  <button onClick={(e) => { e.stopPropagation(); toggleTaskStatus(t); }} className={`w-7 h-7 rounded-xl border-2 flex items-center justify-center shrink-0 transition-all ${t.status === TaskStatus.DONE ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-[var(--border-color)] bg-[var(--bg-card)] group-hover:border-[var(--primary)]'}`}>
                    <i className="fa-solid fa-check text-xs"></i>
                  </button>
                  <div className="flex-1 min-w-0 text-left">
                    <div className="flex items-center gap-2 min-w-0">
                      {t.recurrence && t.recurrence !== 'none' && <i className="fa-solid fa-arrows-rotate text-[10px] shrink-0 text-indigo-400"></i>}
      ...
      <button onClick={onShowScheduleMobile} className="lg:hidden fixed bottom-24 right-6 w-14 h-14 rounded-full bg-[var(--text-main)] text-[var(--bg-main)] shadow-2xl flex items-center justify-center z-40 hover:scale-110 active:scale-95 transition-all border-4 border-[var(--bg-card)]"><i className="fa-solid fa-clock text-xl"></i></button>
    </div>
  );
};
