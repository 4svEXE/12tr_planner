      ...
      <div className="flex-1 overflow-x-auto custom-scrollbar relative flex flex-col">
        <div className="min-w-max flex flex-col flex-1">
          <div className="flex sticky top-0 z-40 bg-[var(--bg-card)] border-b border-[var(--border-color)]">
            <div style={{ width: HOUR_COLUMN_WIDTH }} className="sticky left-0 z-50 bg-[var(--bg-card)] border-r border-[var(--border-color)] shrink-0" />
            {weekDays.map((d, i) => {
              const isToday = d.toDateString() === new Date().toDateString();
              return (
                <div key={i} style={{ width: `calc((100vw - ${HOUR_COLUMN_WIDTH}px - 48px) / 3)` }} className="min-w-[100px] md:min-w-[140px] p-2 md:p-3 text-center border-r border-[var(--border-color)] last:border-r-0 shrink-0">
          ...
          <div className="flex flex-1 relative overflow-y-auto no-scrollbar" style={{ maxHeight: 'calc(100vh - 250px)' }}>
            ...
            {weekDays.map((date, dayIdx) => {
              ...
              return (
                <div key={dayIdx} style={{ width: `calc((100vw - ${HOUR_COLUMN_WIDTH}px - 48px) / 3)` }} onDragOver={(e) => onDragOver(e, dayTs)} onDragLeave={onDragLeave} onDrop={(e) => onDrop(e, dayTs)} onClick={() => onAddQuickEvent(dayTs)} className={`flex-1 min-w-[100px] md:min-w-[140px] border-r border-[var(--border-color)] last:border-r-0 relative group cursor-pointer transition-colors shrink-0 ${dragOverDay === dayTs ? 'bg-[var(--primary)]/5 ring-inset ring-2 ring-[var(--primary)]/20' : 'hover:bg-[var(--bg-main)]/10'}`}>
                  {Array.from({ length: 24 }, (_, i) => (
                    <div key={i} className="h-[60px] border-b border-[var(--border-color)]/20"></div>
                  ))}
                  <div className="absolute inset-0 p-1 md:p-2 space-y-1">
                    {personEvents.map(pe => (
                      <div key={pe.id} className="p-1 rounded-md border text-[7px] md:text-[9px] font-black shadow-sm bg-amber-500/10 border-amber-500/20 text-amber-600 truncate">{pe.title}</div>
                    ))}
                    {dayTasks.map(t => (
                      <div key={t.id} draggable onDragStart={(e) => e.dataTransfer.setData('taskId', t.id)} onClick={(e) => { e.stopPropagation(); onSelectTask(t.id); }} className={`p-1.5 md:p-2 rounded-lg md:rounded-xl border text-[8px] md:text-[10px] font-bold shadow-sm transition-all hover:scale-[1.02] cursor-grab active:cursor-grabbing ${t.isEvent ? 'bg-[var(--primary)] text-white border-transparent' : 'bg-[var(--bg-card)] border-[var(--border-color)] text-[var(--text-main)] hover:border-[var(--primary)]/50'}`}>
                        <div className="flex items-center gap-1 min-w-0">
                           {t.recurrence && t.recurrence !== 'none' && <i className="fa-solid fa-arrows-rotate text-[7px] shrink-0 opacity-60"></i>}
