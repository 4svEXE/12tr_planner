...
const ListsSidebar: React.FC<ListsSidebarProps> = ({ selectedProjectId, onSelectProject, onOpenListModal, isMobile, onClose }) => {
  ...
  const [isNotebookExpanded, setIsNotebookExpanded] = useState(() => {
    const saved = localStorage.getItem('12tr_sidebar_notebook_expanded');
    return saved !== null ? JSON.parse(saved) : true;
  });
  ...
  useEffect(() => {
    localStorage.setItem('12tr_sidebar_notebook_expanded', JSON.stringify(isNotebookExpanded));
  }, [isNotebookExpanded]);
  ...
  return (
    <aside className="w-full h-full bg-[var(--bg-sidebar)] border-r border-[var(--border-color)] flex flex-col shadow-xl md:shadow-none overflow-hidden">
      <header
        ...
        className="p-4 md:p-5 border-b border-[var(--border-color)] flex justify-between items-center bg-transparent shrink-0 cursor-pointer hover:bg-black/5 transition-colors group/nb"
      >
        <div className="flex items-center gap-2">
          <i className={`fa-solid fa-chevron-right text-[10px] transition-transform duration-300 ${isNotebookExpanded ? 'rotate-90' : ''} text-[var(--text-sidebar)] opacity-30 group-hover/nb:opacity-100`}></i>
          <Typography variant="h2" className="text-lg font-black uppercase tracking-tight text-[var(--text-sidebar)]">Записник</Typography>
        </div>
        {isMobile && (
          <button onClick={(e) => { e.stopPropagation(); onClose?.(); }} className="w-10 h-10 rounded-2xl bg-black/5 flex items-center justify-center active:scale-90 transition-all">
      ...
      {isNotebookExpanded && (
        <div className="flex-1 flex flex-col overflow-hidden animate-in fade-in slide-in-from-top-1 duration-300">
          <div className="flex-1 overflow-y-auto custom-scrollbar px-2 space-y-6 pt-4">
