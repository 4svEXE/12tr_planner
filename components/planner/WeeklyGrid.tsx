      ...
      {shortDays.map((dayLabel, idx) => {
        ...
        return (
          <div key={idx} onDragOver={(e) => { e.preventDefault(); setDragOverDay(idx); }} onDragLeave={() => setDragOverDay(null)} onDrop={(e) => handleDrop(e, idx)} className={`flex flex-col gap-1 transition-all min-w-[140px] ${isToday ? 'z-10' : ''}`}>
            ...
            <div className={`rounded-lg py-1 px-3 text-center border transition-all flex items-center justify-between h-8 ${isToday ? 'bg-primary border-primary text-white shadow-md' : 'bg-slate-900 border-white/5 text-slate-400'}`}>
               <div className="text-[11px] font-black uppercase tracking-tight">{dayLabel}</div>
               <div className={`text-[11px] font-black ${isToday ? 'text-white' : 'text-slate-500'}`}>{dateStr.split('-')[2]}</div>
            </div>
            ...
            <Card padding="none" className={`flex-1 bg-card border-theme rounded-lg min-h-[480px] flex flex-col overflow-hidden shadow-sm transition-all ${isToday ? 'ring-2 ring-primary/20 border-primary/40' : ''} ${dragOverDay === idx ? 'bg-primary/5' : ''}`}>
              ...
              <div className="p-2 bg-black/[0.02] border-b border-theme/50 min-h-[60px]">
                 <div className="flex items-center justify-between mb-1.5 px-1">
                    <span className="text-[7px] font-black uppercase text-orange-500 tracking-widest">Focus</span>
                    {!focusTask && (
                       <button onClick={() => setActiveInput({ day: idx, type: 'focus' })} className="w-4 h-4 rounded-full bg-orange-500/10 text-orange-600 flex items-center justify-center hover:bg-orange-500 hover:text-white transition-all">
                 ...
                 {activeInput?.day === idx && activeInput.type === 'focus' ? (
                   ...
                   <div draggable onDragStart={(e) => handleDragStart(e, focusTask.id)} className="bg-gradient-to-br from-orange-500 to-rose-500 p-2 rounded-md shadow-md relative group/f overflow-hidden cursor-grab active:cursor-grabbing">
                      <div className="flex items-start gap-2 relative z-10">
                         <button onClick={() => onToggleTask(focusTask)} className={`w-4 h-4 rounded border shrink-0 flex items-center justify-center transition-all ${focusTask.status === TaskStatus.DONE ? 'bg-white border-white text-orange-500' : 'bg-transparent border-white/40'}`}>
                           {focusTask.status === TaskStatus.DONE && <i className="fa-solid fa-check text-[8px]"></i>}
                         </button>
                         <div className="flex-1 min-w-0" onClick={() => onSelectHabit?.(focusTask.id)}>
                            <div className={`text-[10px] font-black uppercase leading-tight text-white line-clamp-2 ${focusTask.status === TaskStatus.DONE ? 'line-through opacity-60' : ''}`}>
                              {focusTask.title}
                            </div>
                         </div>
                      </div>
              ...
              <div className="p-2 space-y-4 flex-1 overflow-y-auto no-scrollbar">
                ...
                <div className="space-y-1.5">
                   <div className="flex items-center justify-between px-1">
                      <span className="text-[7px] font-black uppercase text-slate-300 tracking-widest">Tasks</span>
                      <button onClick={() => setActiveInput({ day: idx, type: 'quest' })} className="w-3.5 h-3.5 rounded-full bg-slate-50 text-slate-400 flex items-center justify-center hover:bg-primary hover:text-white transition-all"><i className="fa-solid fa-plus text-[7px]"></i></button>
                   </div>
                   <div className="space-y-1">
                    {dayTasks.map(t => (
                      <div key={t.id} draggable onDragStart={(e) => handleDragStart(e, t.id)} className={`flex items-center gap-2 p-1.5 rounded border transition-all cursor-grab active:cursor-grabbing ${t.status === TaskStatus.DONE ? 'bg-emerald-50/30 border-emerald-100 opacity-60' : 'bg-white border-slate-50 hover:border-primary/10 shadow-sm'}`}>
                          <button onClick={() => onToggleTask(t)} className={`w-3.5 h-3.5 rounded border shrink-0 flex items-center justify-center transition-all ${t.status === TaskStatus.DONE ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-slate-200 bg-white'}`}>
                            {t.status === TaskStatus.DONE && <i className="fa-solid fa-check text-[7px]"></i>}
                          </button>
                          <span className={`flex-1 text-[10px] font-bold truncate leading-none ${t.status === TaskStatus.DONE ? 'line-through text-slate-400' : 'text-slate-700'}`}>{t.title}</span>
                      </div>
                    ))}
                   </div>
                </div>
                ...
                <div className="space-y-1.5">
                   <div className="flex items-center justify-between px-1">
                      <span className="text-[7px] font-black uppercase text-indigo-300 tracking-widest">Rituals</span>
                      <button onClick={() => setActiveInput({ day: idx, type: 'recurrent' })} className="w-3.5 h-3.5 rounded-full bg-indigo-50 text-indigo-400 flex items-center justify-center hover:bg-indigo-500 hover:text-white transition-all"><i className="fa-solid fa-plus text-[7px]"></i></button>
                   </div>
                   <div className="space-y-1">
                    {recurrentTasks.map(t => {
                      const isDoneToday = t.habitHistory?.[dateStr]?.status === 'completed';
                      return (
                        <div key={t.id} className={`flex items-center gap-2 p-1.5 rounded transition-all border ${isDoneToday ? 'bg-indigo-50/30 border-indigo-100 opacity-60' : 'bg-white border-slate-50 hover:border-indigo-100 shadow-sm'}`}>
                            <button onClick={(e) => { e.stopPropagation(); toggleHabitStatus(t.id, dateStr, isDoneToday ? 'none' : 'completed'); }} className={`w-3.5 h-3.5 rounded border shrink-0 flex items-center justify-center transition-all ${isDoneToday ? 'bg-indigo-500 border-indigo-500 text-white' : 'border-slate-200 bg-white'}`}>
                              {isDoneToday && <i className="fa-solid fa-check text-[7px]"></i>}
                            </button>
                            <span onClick={() => onSelectHabit?.(t.id)} className={`flex-1 text-[10px] font-bold truncate leading-none cursor-pointer hover:text-indigo-600 ${isDoneToday ? 'line-through text-slate-400' : 'text-slate-700'}`}>{t.title}</span>
                        </div>
                      );
                    })}
                   </div>
