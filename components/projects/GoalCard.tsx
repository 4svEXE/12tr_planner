    ...
    <Card padding="none" className={`bg-card border-theme shadow-sm overflow-hidden transition-all ${isExpanded ? 'shadow-lg ring-2 ring-primary/10' : 'hover:border-primary/30'}`}>
      ...
      {isExpanded && (
        <div className="px-4 pb-6 pt-1 bg-black/[0.02] border-t border-theme animate-in slide-in-from-top-2 duration-300">
           ...
           <div className="flex justify-between items-center mb-4 overflow-x-auto no-scrollbar pb-1 pt-2">
              <div className="flex gap-2 bg-main p-1.5 rounded-2xl border border-theme shrink-0">
                {[{ id: 'actions', label: 'Дії', icon: 'fa-bolt' }, { id: 'subprojects', label: 'Етапи', icon: 'fa-layer-group' }, { id: 'habits', label: 'Звички', icon: 'fa-repeat' }].map(t => (
                  <button key={t.id} onClick={() => setActiveTab(t.id as any)} className={`px-4 py-2.5 text-[9px] font-black uppercase tracking-widest transition-all rounded-xl flex items-center gap-2 ${activeTab === t.id ? 'bg-card shadow-md text-primary' : 'text-muted hover:text-main'}`}>
