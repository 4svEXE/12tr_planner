    ...
    <div className="min-h-screen bg-[var(--bg-main)] pb-32">
      ...
      <div className="bg-white border-b border-slate-100 px-6 py-8 relative overflow-hidden">
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center gap-8 relative z-10">
          <div className="w-28 h-28 md:w-32 md:h-32 rounded-[2.5rem] bg-white p-1 shadow-2xl ring-4 ring-orange-50 relative overflow-hidden">
            <img src={character.avatarUrl} alt="Hero" className="w-full h-full object-cover rounded-[2rem] bg-slate-50" />
          </div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        {activeTab === 'identity' && (
          <div className="space-y-8 animate-in fade-in duration-300">
             ...
             <section>
                <Typography variant="tiny" className="text-slate-900 font-black uppercase flex items-center gap-2 mb-4">
                   <i className="fa-solid fa-sliders text-emerald-500"></i> Системні вподобання
                </Typography>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                   <Card padding="md">
                      <label className="text-[8px] font-black uppercase text-slate-400 block mb-2">Стиль роботи</label>
                      <input value={character.preferences.workStyle} onChange={e => updateCharacter({ preferences: { ...character.preferences, workStyle: e.target.value } })} className="w-full bg-slate-50 border-none rounded-xl p-3 text-xs font-bold outline-none" />
                   </Card>
                   <Card padding="md">
                      <label className="text-[8px] font-black uppercase text-slate-400 block mb-2">Стиль планування</label>
                      <input value={character.preferences.planningStyle} onChange={e => updateCharacter({ preferences: { ...character.preferences, planningStyle: e.target.value } })} className="w-full bg-slate-50 border-none rounded-xl p-3 text-xs font-bold outline-none" />
                   </Card>
                   <Card padding="md">
                      <label className="text-[8px] font-black uppercase text-slate-400 block mb-2">Фокус-блоккери</label>
                      <div className="flex flex-wrap gap-1.5">
                         {character.preferences.focusBlockers.map((b, i) => <Badge key={i} variant="rose" className="text-[7px]">{b}</Badge>)}
                      </div>
                      <button onClick={() => { const b = prompt('Новий блоккер:'); if(b) updateCharacter({ preferences: { ...character.preferences, focusBlockers: [...character.preferences.focusBlockers, b] } }); }} className="mt-2 text-[8px] font-black text-rose-600 uppercase tracking-widest">+ Додати</button>
                   </Card>
                </div>
             </section>
          </div>
        ...
        {activeTab === 'evolution' && (
          <div className="space-y-8 animate-in fade-in duration-300">
             ...
             <section className="space-y-4">
                <Typography variant="h2" className="text-xl font-black">Трофейний зал</Typography>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                   {character.achievements.map(ach => (
                      <Card key={ach.id} padding="md" className="border-slate-100 text-center hover:border-orange-200 transition-all group">
                         <div className="w-12 h-12 rounded-full bg-orange-50 text-orange-600 flex items-center justify-center text-xl mx-auto mb-3 shadow-inner group-hover:scale-110 transition-transform">
                            <i className={`fa-solid ${ach.icon}`}></i>
                         </div>
                         <div className="text-[10px] font-black uppercase text-slate-800 mb-1">{ach.title}</div>
                         <p className="text-[9px] text-slate-400 leading-tight">{ach.description}</p>
                      </Card>
                   ))}
                   <div className="border-2 border-dashed border-slate-100 rounded-[2rem] flex flex-col items-center justify-center p-6 grayscale opacity-30">
                      <i className="fa-solid fa-lock mb-2 text-slate-300"></i>
                      <span className="text-[8px] font-black uppercase tracking-widest">Locked Achievement</span>
                   </div>
                </div>
             </section>
          </div>
