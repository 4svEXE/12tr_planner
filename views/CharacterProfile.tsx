...
const CharacterProfile: React.FC = () => {
   const { character, updateCharacter, tasks, projects, cycle, diary, hobbies } = useApp();
   ...
   const achievementsList = useMemo(() => {
      return [
         { id: 'first_blood', title: 'Перша Кров', desc: 'Виконати своє перше завдання', icon: 'fa-droplet', unlocked: gtdStats.done >= 1 },
         { id: 'apprentice', title: 'Початківець', desc: 'Накопичити 500 XP', icon: 'fa-star', color: 'text-amber-500', bg: 'bg-amber-500/10', unlocked: character.xp >= 500 },
         { id: 'master', title: 'Майстер квестів', desc: 'Виконати 50 завдань', icon: 'fa-medal', color: 'text-rose-500', bg: 'bg-rose-500/10', unlocked: gtdStats.done >= 50 },
         { id: 'rich', title: 'Скарбничий', desc: 'Накопичити 1000 Золота', icon: 'fa-coins', color: 'text-yellow-500', bg: 'bg-yellow-500/10', unlocked: character.gold >= 1000 },
         { id: 'veteran', title: 'Ветеран', desc: 'Досягти 10-го рівня', icon: 'fa-crown', color: 'text-indigo-500', bg: 'bg-indigo-500/10', unlocked: character.level >= 10 },
         { id: 'planner', title: 'Архітектор Долі', desc: 'Створити 5 проектів', icon: 'fa-chess-knight', color: 'text-emerald-500', bg: 'bg-emerald-500/10', unlocked: projects.length >= 5 },
         { id: 'focused', title: 'Монах', desc: 'Досягнути 100% фокусу', icon: 'fa-yin-yang', color: 'text-cyan-500', bg: 'bg-cyan-500/10', unlocked: character.focus >= 100 },
         { id: 'diarist', title: 'Літописець', desc: 'Зробити 5 записів у щоденнику', icon: 'fa-book-journal-whills', color: 'text-purple-500', bg: 'bg-purple-500/10', unlocked: diary && diary.length >= 5 },
      ];
   }, [gtdStats.done, character.xp, character.gold, character.level, projects.length, character.focus, diary]);
   ...
   const renderTabTrigger = (id: ProfileTab, label: string, icon: string) => (
      <button
         ...
         className={`flex items-center gap-2 px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === id ? 'bg-slate-900 text-white shadow-xl' : 'text-slate-400 hover:bg-slate-100'
            }`}
      >
               ...
               <div className="space-y-8 animate-in fade-in duration-300">
                  ...
                  <section className="space-y-6">
                     <div className="flex items-center justify-between">
                        <Typography variant="h2" className="text-xl font-black uppercase">Зал Слави</Typography>
                        <Badge variant="orange">{achievementsList.filter(a => a.unlocked).length} / {achievementsList.length}</Badge>
                     </div>
                     <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                        {achievementsList.map(a => (
                           <Card key={a.id} padding="md" className={`flex flex-col items-center justify-center text-center transition-all duration-500 ${a.unlocked ? 'border-amber-200 bg-amber-50/30 dark:bg-amber-900/10' : 'border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/5 grayscale opacity-40'}`}>
                              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl mb-4 ${a.unlocked ? `${a.bg || 'bg-orange-500/10'} ${a.color || 'text-orange-500'}` : 'bg-slate-200 dark:bg-white/10 text-slate-400 dark:text-white/40'}`}>
                                 <i className={`fa-solid ${a.icon} ${a.unlocked ? 'drop-shadow-sm' : ''}`}></i>
                              </div>
                              <Typography variant="tiny" className={`font-black uppercase tracking-widest text-[9px] mb-1 ${a.unlocked ? 'text-[var(--text-main)]' : 'text-[var(--text-muted)]'}`}>{a.title}</Typography>
                              <p className="text-[10px] font-medium text-[var(--text-muted)] leading-tight">{a.desc}</p>
                           </Card>
                        ))}
                     </div>
                  </section>
