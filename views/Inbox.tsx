  ...
  const renderTask = (task: Task) => {
    ...
    return (
      <div key={task.id} 
        ...
        className={`group flex items-center gap-3 py-3 md:py-2 px-4 md:px-6 hover:bg-black/5 cursor-pointer md:cursor-grab active:cursor-grabbing border-b border-[var(--border-color)]/30 transition-colors ${selectedTaskId === task.id ? 'bg-[var(--sidebar-item-active)] border-l-2 border-l-[var(--primary)]' : 'border-l-2 border-l-transparent'} ${isDone ? 'opacity-40' : ''}`}
      >
        <button onClick={(e) => { e.stopPropagation(); toggleTaskStatus(task); }} className={`w-6 h-6 md:w-5 md:h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${isDone ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-slate-200 bg-white text-transparent hover:border-[var(--primary)]'}`}> {isDone && <i className="fa-solid fa-check text-[10px]"></i>} </button>
        <div className="flex-1 min-w-0 flex items-center justify-between gap-3">
          <div className="flex-1 min-w-0">
            {isEditing ? (
              <input autoFocus value={editingValue} onChange={e => setEditingValue(e.target.value)} onBlur={() => { if (editingValue.trim() && editingValue !== task.title) updateTask({ ...task, title: editingValue.trim() }); setEditingTaskId(null); }} onKeyDown={e => { if (e.key === 'Enter') e.currentTarget.blur(); }} className="w-full bg-transparent border-none p-0 text-[14px] md:text-[13px] font-bold text-[var(--text-main)] focus:ring-0 h-5 outline-none" onClick={e => e.stopPropagation()} />
            ) : (
              <div onDoubleClick={(e) => { e.stopPropagation(); setEditingTaskId(task.id); setEditingValue(task.title); }} className={`text-[14px] md:text-[13px] font-bold truncate block leading-tight ${isDone ? 'line-through text-[var(--text-muted)]' : 'text-[var(--text-main)]'}`}> {task.title} </div>
            )}
            {project && <span className="text-[8px] md:text-[7px] font-black uppercase text-orange-400 opacity-70 px-1.5 py-0.5 bg-orange-50 rounded mt-1 inline-block"># {project.name}</span>}
          </div>
          <div className="flex gap-1.5 shrink-0 ml-2"> {task.tags.slice(0, isMobile ? 1 : undefined).map(tagName => { const tagObj = tags.find(t => t.name === tagName); return ( <span key={tagName} className="text-[10px] md:text-[9px] font-black uppercase tracking-tight" style={{ color: tagObj?.color || 'var(--primary)' }}> #{tagName} </span> ); })} </div>
        </div>
      </div>
    );
  };
    ...
    <div className="h-screen flex flex-col lg:flex-row overflow-hidden relative bg-[var(--bg-main)]">
        <div className="flex-1 flex flex-col min-w-0 h-full relative">
          <header className="px-4 md:px-6 pt-4 md:pt-3 pb-2 md:pb-1 border-b border-[var(--border-color)] flex flex-col gap-3 md:gap-2 sticky top-0 bg-[var(--bg-card)] z-20 shadow-sm shrink-0">
            <div className="flex items-center justify-between h-8">
              <div className="flex items-center gap-3 md:gap-4 h-full">
                <Typography variant="h1" className="text-sm md:text-base text-[var(--text-main)] tracking-tight font-black uppercase flex items-center gap-2 md:gap-3">
                ...
                <Badge variant="orange" className="px-2 py-0.5 rounded-lg text-[10px] md:text-[9px] bg-[var(--primary)]/10 text-[var(--primary)]">{displayedTasks.length}</Badge>
                {!showCompleted && (
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar pb-32 mt-1">
            {(displayCategories as InboxCategory[]).map(section => {
                ...
                return (
                  <div key={section.id} 
                    draggable={!isMobile && !isProject && !['pinned', 'unsorted', 'tasks', 'notes'].includes(section.id)} 
                  ...
                  >
                    <div className="group flex items-center gap-3 py-3 md:py-2.5 px-4 md:px-6 hover:bg-black/5 cursor-pointer select-none">
                      <div className="flex items-center gap-3 md:gap-2.5 flex-1 min-w-0" onClick={() => setCollapsedSections(p => ({...p, [section.id]: !isCollapsed}))}>
                        <i className={`fa-solid fa-chevron-right text-[10px] md:text-[8px] text-[var(--text-muted)] transition-transform ${!isCollapsed ? 'rotate-90' : ''}`}></i>
                        {isProject && (section as any).projectColor ? ( <div className="w-1.5 h-5 md:w-1 md:h-4 rounded-full" style={{ backgroundColor: (section as any).projectColor }}></div> ) : ( <i className={`fa-solid ${section.icon} text-[13px] md:text-[10px] shrink-0 ${!isProject ? sectionColorData.text : 'text-slate-900'}`}></i> )}
                        <div className="flex-1 min-w-0">
                           {isEditingName ? ( <div className="flex flex-col gap-2" onClick={e => e.stopPropagation()}> <input autoFocus value={sectionEditTitle} onChange={e => setSectionEditTitle(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSaveSectionRename(section.id)} className="bg-[var(--bg-main)] border border-[var(--primary)]/30 rounded px-1.5 py-1 text-[11px] font-black uppercase w-full outline-none" /> </div> ) : ( <Typography variant="tiny" onDoubleClick={(e) => { e.stopPropagation(); if (!isProject && !['pinned', 'unsorted', 'tasks', 'notes'].includes(section.id)) { setEditingSectionId(section.id); setSectionEditTitle(section.title); setSectionEditColor(section.color || 'slate'); } }} className={`${isProject ? 'text-[var(--text-main)] font-black' : `${sectionColorData.text} font-black`} uppercase tracking-widest text-[10px] md:text-[9px] truncate`}>{section.title}</Typography> )}
                        </div>
                        {!showCompleted && !isEditingName && (
                           <div className={`flex items-center gap-2 md:gap-1.5 ${isMobile ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                             <button onClick={(e) => { e.stopPropagation(); setAddingTaskToSectionId(section.id); }} className="w-7 h-7 md:w-5 md:h-5 rounded-lg bg-[var(--primary)]/10 text-[var(--primary)] flex items-center justify-center hover:scale-110 shadow-sm"><i className="fa-solid fa-plus text-[11px] md:text-[9px]"></i></button>
                             {!isProject && !['pinned', 'unsorted', 'tasks', 'notes'].includes(section.id) && ( <> <button onClick={(e) => { e.stopPropagation(); setEditingSectionId(section.id); setSectionEditTitle(section.title); setSectionEditColor(section.color || 'slate'); }} className="w-7 h-7 md:w-5 md:h-5 rounded-lg hover:bg-orange-50 text-[var(--primary)] flex items-center justify-center"><i className="fa-solid fa-pencil text-[11px] md:text-[9px]"></i></button> <button onClick={(e) => { e.stopPropagation(); setDeletingSection(section); }} className="w-7 h-7 md:w-5 md:h-5 rounded-lg hover:bg-rose-50 text-rose-500 flex items-center justify-center"><i className="fa-solid fa-trash text-[12px] md:text-[10px]"></i></button> </> )}
                           </div>
                        )}
                      </div>
                      <span className="text-[10px] md:text-[9px] font-black text-[var(--text-muted)] opacity-30">{sectionTasks.length}</span>
                    </div>
                    {isAddingTask && ( <div className="px-4 md:px-6 py-2 bg-black/5"> <HashtagAutocomplete autoFocus value={sectionTaskTitle} onChange={setSectionTaskTitle} onSelectTag={() => {}} onBlur={() => { if(!sectionTaskTitle.trim()) setAddingTaskToSectionId(null); }} onEnter={() => handleAddToSection(section.id, !!isProject)} placeholder="Швидка дія..." className="w-full bg-[var(--bg-main)] border border-[var(--primary)]/30 rounded-xl px-4 py-2 text-[12px] md:text-[11px] font-bold outline-none h-11 md:h-10" /> </div> )}
          ...
          {aiEnabled && !showNextActions && !showCompleted && (
            <div className={`absolute left-1/2 -translate-x-1/2 z-30 transition-all ${isMobile ? 'bottom-20' : 'bottom-6'}`}>
               <button onClick={handleAiProcess} disabled={isAiProcessing} className="bg-slate-900 text-white px-6 md:px-8 py-3.5 md:py-3 rounded-2xl shadow-2xl flex items-center gap-3 hover:scale-105 active:scale-95 transition-all group overflow-hidden border border-white/10"> <div className="absolute inset-0 bg-gradient-to-tr from-orange-600/20 to-pink-600/20 opacity-0 group-hover:opacity-100 transition-opacity"></div> {isAiProcessing ? ( <i className="fa-solid fa-circle-notch animate-spin text-orange-400"></i> ) : ( <i className="fa-solid fa-sparkles text-orange-400"></i> )} <span className="text-[11px] md:text-[10px] font-black uppercase tracking-widest relative z-10"> {isAiProcessing ? 'РОЗБИРАЮ...' : 'РОЗІБРАТИ ВХІДНІ З ШІ'} </span> </button>
            </div>
          )}
        </div>
        
        {/* RIGHT SIDEBAR / DETAILS - Adaptive */}
        <div className={`flex h-full border-l border-[var(--border-color)] z-[110] bg-[var(--bg-card)] shrink-0 transition-all duration-300 ${isMobile ? (selectedTaskId ? 'fixed inset-0 w-full translate-x-0' : 'fixed inset-0 w-full translate-x-full') : ''}`}>
          ...
          <div style={{ width: isMobile ? '100%' : detailsWidth }} className="h-full bg-[var(--bg-card)] relative overflow-hidden flex flex-col">
            {selectedTaskId ? ( <TaskDetails task={tasks.find(t => t.id === selectedTaskId)!} onClose={() => setSelectedTaskId(null)} /> ) : ( !isMobile && <div className="h-full flex flex-col items-center justify-center p-8 text-center opacity-5 grayscale pointer-events-none"> <i className="fa-solid fa-bolt-lightning text-8xl mb-8"></i> <Typography variant="tiny" className="text-[12px] font-black uppercase tracking-[0.3em]">Фокус на дії</Typography> </div> )}
          </div>
        </div>
