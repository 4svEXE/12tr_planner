    ...
    <div className="fixed inset-0 z-[3000] flex items-center justify-center p-4 animate-in fade-in duration-200">
      ...
      <Card className="w-full max-w-sm relative z-10 shadow-2xl p-6 md:p-8 rounded-[2.5rem] bg-card border-theme animate-in zoom-in-95 duration-200">
        <header className="flex justify-between items-center mb-6">
          <Typography variant="h2" className="text-xl font-bold uppercase tracking-tight text-main">Новий Союзник</Typography>
        ...
        <div className="space-y-6">
          <div className="space-y-1.5">
            <label className="text-[9px] font-bold uppercase text-muted mb-1 block ml-0.5 tracking-[0.1em] opacity-60">Повне ім'я</label>
            <input
              autoFocus
              value={name}
          ...
          <div className="space-y-1.5">
            <label className="text-[9px] font-bold uppercase text-muted mb-1 block ml-0.5 tracking-[0.1em] opacity-60">Статус зв'язку</label>
            <select
              value={status}
          ...
          <div className="flex gap-3 pt-4">
            <button onClick={onClose} className="flex-1 py-3.5 rounded-2xl font-bold uppercase tracking-widest text-[10px] text-muted hover:bg-black/5 transition-all border border-theme">Скасувати</button>
            <button
              onClick={handleSave}
              disabled={!name.trim()}
              className="flex-[2] py-3.5 bg-primary text-white rounded-2xl font-bold uppercase tracking-widest text-[10px] shadow-xl shadow-primary/20 hover:brightness-110 active:scale-95 transition-all disabled:opacity-30 disabled:grayscale"
            >
              Створити
            </button>
          </div>
        ...
        <header className="p-4 md:p-6 bg-card border-b border-theme z-20 flex flex-col shrink-0">
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <Typography variant="h2" className="text-lg md:text-xl leading-none font-bold uppercase tracking-tight">Мережа</Typography>
            ...
            <div className="flex items-center gap-2">
              ...
              <Button onClick={() => setIsAdding(true)} icon="fa-plus" size="sm" className="rounded-xl px-4 text-[9px] font-bold uppercase">ДОДАТИ</Button>
            </div>
          </div>
          ...
          <div className="max-w-6xl mx-auto">
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                  ...
                  <Card key={person.id} padding="none" onClick={() => { setSelectedPersonId(person.id); setInitialTab('dossier'); }}
                    ...
                    <div className="p-5 flex gap-4 items-start">
                      ...
                      <div className="flex-1 min-w-0">
                        <Typography variant="h3" className="text-sm font-bold uppercase truncate text-main mb-1">{person.name}</Typography>
              ...
              {!isMobile && (
                <button onClick={() => setIsAdding(true)} className="border-2 border-dashed border-theme rounded-2xl p-6 flex flex-col items-center justify-center text-muted hover:border-primary/50 hover:text-primary hover:bg-card transition-all min-h-[100px] group">
                  ...
                  <span className="text-[8px] font-bold uppercase tracking-widest">Додати союзника</span>
                </button>
              )}
            </div>
            ...
            {filteredPeople.length === 0 && searchQuery && (
              <div className="py-20 text-center opacity-20 grayscale flex flex-col items-center select-none pointer-events-none">
                ...
                <Typography variant="h2" className="text-xl text-main font-bold uppercase tracking-widest">Нікого не знайдено</Typography>
              </div>
            )}
          </div>
        </div>
