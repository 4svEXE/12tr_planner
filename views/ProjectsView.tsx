...
const GoalModal: React.FC<{ 
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <Card className="w-full max-w-lg relative z-10 shadow-2xl p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] bg-white animate-in zoom-in-95 duration-200 border-none overflow-y-auto max-h-[90vh]">
        <Typography variant="h2" className="mb-6 text-xl md:text-2xl">{initialData ? 'Редагувати Ціль' : 'Нова Стратегічна Ціль'}</Typography>
        <div className="space-y-4 md:space-y-6">
          <div>
            <label className="text-[9px] md:text-[10px] font-black uppercase text-slate-400 mb-2 block tracking-widest">Назва цілі</label>
            <input autoFocus value={name} onChange={e => setName(e.target.value)} placeholder="Напр: Досягти фінансової свободи" className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-3 md:py-4 px-4 md:px-5 text-sm font-bold focus:ring-4 focus:ring-orange-100 outline-none transition-all" />
          </div>
          <div>
            <label className="text-[9px] md:text-[10px] font-black uppercase text-slate-400 mb-2 block tracking-widest">Сфера життя</label>
            <div className="grid grid-cols-3 gap-2">
              {spheres.map(s => (
                <button key={s.key} onClick={() => setSphere(s.key as any)} className={`p-2 md:p-3 rounded-xl border-2 transition-all flex flex-col items-center gap-1 ${sphere === s.key ? 'border-orange-500 bg-orange-50 text-orange-700' : 'border-slate-50 bg-slate-50 text-slate-400 hover:border-slate-100'}`}>
                  <i className={`fa-solid ${s.icon} text-[10px] md:text-xs`}></i>
                  <span className="text-[7px] md:text-[8px] font-black uppercase">{s.label}</span>
                </button>
              ))}
            </div>
          </div>
          <div>
            <label className="text-[9px] md:text-[10px] font-black uppercase text-slate-400 mb-2 block tracking-widest">Візія та опис</label>
            <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Опишіть бажаний результат..." className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-3 md:py-4 px-4 md:px-5 text-sm font-bold focus:ring-4 focus:ring-orange-100 outline-none transition-all h-20 md:h-24 resize-none" />
          </div>
          {aiEnabled && !initialData && (
             <div className="p-3 md:p-4 bg-indigo-50 rounded-2xl border border-indigo-100 flex items-center justify-between">
                <div className="flex items-center gap-3">
                   <div className="w-8 h-8 md:w-10 md:h-10 rounded-xl bg-indigo-600 text-white flex items-center justify-center shadow-lg shadow-indigo-200"><i className="fa-solid fa-wand-magic-sparkles text-xs md:text-base"></i></div>
                   <div>
                      <div className="text-[9px] md:text-[10px] font-black uppercase text-indigo-900 leading-none mb-1">AI Планування</div>
                      <div className="text-[7px] md:text-[8px] font-bold text-indigo-400">Створити план дій</div>
                   </div>
                </div>
          ...
          <div className="flex gap-3 md:gap-4 pt-2 md:pt-4">
            <Button variant="ghost" className="flex-1 py-3 md:py-4 rounded-2xl font-black text-xs" onClick={onClose}>СКАСУВАТИ</Button>
            <Button disabled={!name.trim()} className="flex-[2] py-3 md:py-4 rounded-2xl shadow-xl shadow-orange-100 font-black text-xs" onClick={() => onSave({ name, description, color, sphere }, autoPlan)}>
...
const SubProjectPanel: React.FC<{ subProject: Project, onClose: () => void }> = ({ subProject, onClose }) => {
  ...
  return (
    <div onDragOver={(e) => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={handleDrop}
      ...
      <header className="p-4 md:p-6 border-b border-slate-50 flex justify-between items-start">
        <div className="flex items-center gap-3 md:gap-4">
           <div className="w-10 h-10 rounded-2xl text-white flex items-center justify-center text-lg shadow-lg" style={{ backgroundColor: subProject.color }}><i className="fa-solid fa-layer-group"></i></div>
           <div>
              ...
              <Typography variant="h2" className="text-base md:text-lg leading-tight">{subProject.name}</Typography>
           </div>
        </div>
         ...
         <section>
            ...
            <div className="space-y-1.5">
               {spTasks.map(t => {
                 ...
                 return (
                   <div key={t.id} draggable onDragStart={(e) => e.dataTransfer.setData('taskId', t.id)}
                     className={`flex items-center gap-3 p-3 bg-slate-50 border border-slate-100 rounded-xl group hover:bg-white hover:border-orange-200 transition-all ${isDoing ? 'opacity-40 grayscale' : ''}`}>
                      <button onClick={() => toggleTaskStatus(t)} className={`w-5 h-5 rounded-full border flex items-center justify-center transition-all ${t.status === TaskStatus.DONE ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-white border-slate-200'}`}>
                      ...
                      <button onClick={() => deleteTask(t.id)} className="text-slate-300 hover:text-rose-500 transition-all p-1"><i className="fa-solid fa-trash-can text-[10px]"></i></button>
                   </div>
                 );
               })}
               <form onSubmit={e => { e.preventDefault(); if(newAction.trim()){ addTask(newAction.trim(), 'tasks', subProject.id, 'actions'); setNewAction(''); }}} className="pt-2">
                  <input value={newAction} onChange={e=>setNewAction(e.target.value)} placeholder="+ Нова дія..." className="w-full bg-slate-50 rounded-xl px-4 py-3 text-[13px] font-bold outline-none border border-transparent focus:border-orange-300 transition-colors" />
               </form>
            </div>
         </section>
      </div>
      <footer className="p-4 md:p-6 border-t border-slate-50 bg-slate-50/30">
         <Button variant="white" onClick={onClose} className="w-full text-[10px] font-black uppercase py-4 rounded-xl tracking-widest shadow-sm">ЗАКРИТИ</Button>
      </footer>
    </div>
  );
};
    ...
    <Card padding="none" className={`bg-white border-slate-100 shadow-sm overflow-hidden transition-all ${isExpanded ? 'shadow-xl ring-1 ring-orange-100 scale-[1.01]' : 'hover:border-orange-200'}`}>
      <div className="flex items-stretch">
        ...
        <div onClick={onToggle} className="flex-1 p-3 md:p-4 cursor-pointer flex items-center gap-3 md:gap-4 group">
          <div className="w-10 h-10 md:w-11 md:h-11 rounded-xl md:rounded-[1.2rem] flex items-center justify-center text-white shadow-lg shrink-0 transition-transform group-hover:scale-105 relative" style={{ backgroundColor: goal.color }}>
             <i className="fa-solid fa-flag-checkered text-xs md:text-sm"></i>
             {goal.sphere && (
               <div className="absolute -bottom-1 -right-1 w-4 h-4 md:w-4.5 md:h-4.5 rounded-lg bg-white shadow-md flex items-center justify-center text-[7px] md:text-[8px] text-slate-900 border border-slate-50"><i className={`fa-solid ${sphereIcons[goal.sphere] || 'fa-mountain'}`}></i></div>
             )}
          </div>
          <div className="flex-1 min-w-0">
             <div className="flex items-center gap-2 mb-0.5">
               <Typography variant="h3" className="text-sm md:text-[15px] font-black uppercase tracking-tight text-slate-800 truncate">{goal.name}</Typography>
               <Badge variant="orange" className="text-[7px] py-0 px-1">{executionScore}% XP</Badge>
             </div>
             <div className="flex items-center gap-2">
                <span className="text-[7px] md:text-[8px] font-black text-slate-400 uppercase tracking-widest">{goal.sphere || 'Вільна сфера'}</span>
             </div>
          </div>
          <div className="shrink-0 flex items-center gap-2">
             <div className="h-6 w-px bg-slate-100 hidden md:block"></div>
             <i className={`fa-solid fa-chevron-right text-[8px] md:text-[10px] text-slate-300 transition-transform ${isExpanded ? 'rotate-90 text-orange-500' : ''}`}></i>
          </div>
        </div>
      </div>

      {isExpanded && (
        <div className="px-4 md:px-5 pb-4 md:pb-5 pt-2 md:pt-3 bg-slate-50/10 border-t border-slate-50 animate-in slide-in-from-top-2 duration-300">
           <div className="flex justify-between items-center mb-3 md:mb-4 overflow-x-auto no-scrollbar pb-1">
              <div className="flex gap-1 bg-slate-100/50 p-1 rounded-xl border border-slate-100 shrink-0">
                {[{ id: 'actions', label: 'Дії', icon: 'fa-bolt' }, { id: 'subprojects', label: 'Етапи', icon: 'fa-layer-group' }, { id: 'habits', label: 'Звички', icon: 'fa-repeat' }].map(t => (
                  <button key={t.id} onClick={() => setActiveTab(t.id as any)} className={`px-2.5 md:px-3 py-1.5 text-[7px] md:text-[8px] font-black uppercase tracking-widest transition-all rounded-lg flex items-center gap-1 md:gap-1.5 ${activeTab === t.id ? 'bg-white shadow-sm text-orange-600' : 'text-slate-400 hover:text-slate-600'}`}>
                    <i className={`fa-solid ${t.icon} text-[8px] md:text-[9px]`}></i> {t.label}
                  </button>
                ))}
              </div>
              <div className="flex gap-1.5 shrink-0 ml-4">
                 <button onClick={() => onEdit(goal)} className="w-7 h-7 md:w-8 md:h-8 rounded-xl bg-white border border-slate-100 text-slate-400 hover:text-orange-600 hover:border-orange-200 transition-all flex items-center justify-center shadow-sm"><i className="fa-solid fa-gear text-[10px] md:text-xs"></i></button>
                 <button onClick={() => { if(confirm('Видалити ціль?')) deleteProject(goal.id); }} className="w-7 h-7 md:w-8 md:h-8 rounded-xl bg-white border border-slate-100 text-slate-400 hover:text-rose-500 hover:border-rose-200 transition-all flex items-center justify-center shadow-sm"><i className="fa-solid fa-trash-can text-[10px] md:text-xs"></i></button>
              </div>
           </div>

           <div className="space-y-3">
              <form onSubmit={handleInlineSubmit} className="flex gap-2">
                 <input value={inlineInputValue} onChange={e => setInlineInputValue(e.target.value)} placeholder={activeTab === 'actions' ? "Тактичний крок..." : activeTab === 'subprojects' ? "Новий етап..." : "Звичка..."} className="flex-1 bg-white border border-slate-200 rounded-xl py-2 px-3 md:px-4 text-[12px] md:text-[13px] font-bold focus:ring-4 focus:ring-orange-100 outline-none transition-all shadow-inner" />
                 <button type="submit" disabled={!inlineInputValue.trim()} className="w-9 h-9 md:w-10 md:h-10 bg-slate-900 text-white rounded-xl flex items-center justify-center disabled:opacity-30 shadow-lg hover:bg-orange-600 transition-all"><i className="fa-solid fa-plus text-xs"></i></button>
              </form>
              ...
              <div className="flex flex-col gap-1.5">
                   ...
                   return (
                    <div key={task.id} draggable onDragStart={(e) => e.dataTransfer.setData('taskId', task.id)} onClick={() => onTaskClick(task.id)}
                      className={`flex items-center gap-3 p-3 rounded-xl bg-white border group transition-all cursor-pointer ${isDoing ? 'opacity-60 grayscale border-slate-100' : 'border-slate-50 hover:border-orange-200 hover:bg-slate-50 shadow-sm'} ${selectedTaskId === task.id ? 'ring-2 ring-orange-200' : ''}`}>
                       <button onClick={(e) => { e.stopPropagation(); toggleTaskStatus(task); }} className={`w-5 h-5 rounded-full border flex items-center justify-center shrink-0 transition-all ${task.status === TaskStatus.DONE ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-white border-slate-200 hover:border-orange-400'}`}>
                          {task.status === TaskStatus.DONE && <i className="fa-solid fa-check text-[8px]"></i>}
                       </button>
                       <div className="flex-1 min-w-0">
                         <div className={`text-[12px] md:text-[13px] font-bold truncate ${task.status === TaskStatus.DONE ? 'text-slate-300 line-through' : 'text-slate-700'}`}>{task.title}</div>
                 ...
                 {activeTab === 'subprojects' && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                       {subProjects.map(sp => (
                         <div key={sp.id} onClick={() => onSubProjectClick(sp.id)} className="bg-white p-3.5 rounded-[1.5rem] border border-slate-100 shadow-sm hover:shadow-md hover:border-orange-200 transition-all cursor-pointer group flex flex-col justify-between h-20 md:h-24">
                            <div className="flex items-center justify-between">
                               <div className="w-7 h-7 md:w-8 md:h-8 rounded-xl text-white flex items-center justify-center shadow-lg group-hover:rotate-6 transition-transform" style={{ backgroundColor: sp.color || goal.color }}><i className="fa-solid fa-layer-group text-[10px] md:text-xs"></i></div>
                               <Badge variant="orange" className="text-[6px]">{sp.progress}%</Badge>
                            </div>
                            <div>
                               <div className="text-[9px] md:text-[10px] font-black text-slate-800 uppercase truncate mb-1.5">{sp.name}</div>
                 ...
                 {activeTab === 'habits' && goalHabits.map(h => (
                   <div key={h.id} onClick={() => onHabitClick(h.id)} className={`p-3 bg-white border rounded-xl flex items-center justify-between shadow-sm group cursor-pointer transition-all ${selectedHabitId === h.id ? 'border-orange-400 ring-2 ring-orange-50' : 'border-slate-50 hover:border-orange-200'}`}>
                      <div className="flex items-center gap-3">
                         ...
                         <span className="text-[12px] md:text-[13px] font-bold text-slate-700">{h.title}</span>
                      </div>
  ...
  return (
    <div className="h-screen flex bg-slate-50 overflow-hidden relative">
      <div className="flex-1 flex flex-col min-w-0 h-full">
        <header className="px-4 md:px-5 py-3 md:py-4 border-b border-slate-100 bg-white flex flex-col gap-3 md:gap-4 sticky top-0 z-20 shadow-sm shrink-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 md:gap-4 overflow-hidden">
              <div className="flex flex-col shrink-0">
                <Typography variant="h2" className="text-sm md:text-lg font-black uppercase tracking-tight text-slate-900 leading-none">Двигун 12TR</Typography>
                <div className="flex items-center gap-1.5 md:gap-2 mt-1 md:mt-1.5">
                   <div className="px-1.5 py-0.5 bg-orange-600 text-white text-[6px] md:text-[7px] font-black rounded uppercase tracking-widest">Активний</div>
                   <Typography variant="tiny" className="text-slate-300 font-bold text-[6px] md:text-[7px]">Тиждень {cycle.currentWeek}/12</Typography>
                </div>
              </div>
              <div className="h-6 w-px bg-slate-100 mx-0.5 md:mx-1 shrink-0"></div>
              <div className="flex bg-slate-50 p-0.5 rounded-lg border border-slate-200 overflow-x-auto no-scrollbar shrink-0">
                {[{ id: 'strategy', label: isMobile ? '' : 'Стратегія', icon: 'fa-chess-king' }, { id: 'execution', label: isMobile ? '' : 'Виконання', icon: 'fa-bolt-lightning' }, { id: 'structure', label: isMobile ? '' : 'Структура', icon: 'fa-sitemap' }].map(m => (
                  <button key={m.id} onClick={() => setActiveMode(m.id as any)} className={`px-2.5 md:px-3 py-1.5 rounded-md text-[8px] font-black uppercase tracking-widest transition-all flex items-center gap-1.5 ${activeMode === m.id ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}>
                    <i className={`fa-solid ${m.icon} text-[8px] md:text-[9px]`}></i> {m.label}
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="w-full space-y-1">
             <div className="flex justify-between items-center text-[6px] md:text-[7px] font-black uppercase tracking-widest text-slate-400 px-1">
                <span>Прогрес циклу</span>
                <span className="text-orange-600">{cycle.globalExecutionScore}% XP</span>
             </div>
             <div className="flex gap-0.5 h-0.5 md:h-0.5">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar bg-slate-50/50 pb-24 md:pb-32">
          {activeMode === 'structure' ? <StructureView /> : activeMode === 'execution' ? (
             <div className="p-4 md:p-5 max-w-3xl mx-auto space-y-4 md:space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                   <Card className="bg-slate-900 text-white p-4 border-none shadow-xl overflow-hidden relative min-h-[80px] md:min-h-[100px] flex flex-col justify-center"><Typography variant="tiny" className="text-orange-500 mb-0.5 font-black uppercase text-[7px]">Поточний фокус</Typography><Typography variant="h2" className="text-xl md:text-2xl font-black">Тиждень {cycle.currentWeek}</Typography></Card>
                   <Card className="p-4 bg-white border-slate-100 text-center flex flex-col items-center justify-center"><div className="text-xl md:text-2xl font-black text-slate-900 mb-0.5">{cycle.globalExecutionScore}%</div><Typography variant="tiny" className="text-slate-400 font-black uppercase text-[7px]">Загальний KPI</Typography></Card>
                   <Card className="p-4 bg-white border-slate-100 flex flex-col justify-center"><div className="flex justify-between items-center mb-1"><Typography variant="tiny" className="text-slate-400 font-black text-[7px]">STREAK</Typography><span className="text-orange-500 font-black text-[10px] md:text-[11px]">🔥 12</span></div><div className="flex justify-between items-center"><Typography variant="tiny" className="text-slate-400 font-black text-[7px]">КВЕСТІВ</Typography><span className="text-indigo-600 font-black text-[10px] md:text-[11px]">84</span></div></Card>
                </div>
                
                <div className="space-y-4">
                   <Typography variant="h2" className="text-[9px] md:text-[10px] uppercase font-black tracking-widest px-1 text-slate-400">Наступні дії за проєктами</Typography>
                     ...
                     return (
                        <div key={goal.id} className="space-y-1.5">
                           <div className="flex items-center gap-1.5 px-1 mt-2">
                              ...
                              <span className="text-[9px] md:text-[10px] font-black uppercase tracking-tight text-slate-400">{goal.name}</span>
                           </div>
                           <div className="grid grid-cols-1 md:grid-cols-2 gap-1.5">
                              {allVisibleActions.map(t => {
                                const isSub = projects.find(p => p.id === t.projectId && p.type === 'subproject');
                                return (
                                 <Card key={t.id} padding="none" onClick={() => setSelectedTaskId(t.id)} className={`bg-white border-slate-100 hover:border-orange-200 transition-all flex items-center gap-2 p-3 group cursor-pointer ${t.status === TaskStatus.DOING ? 'opacity-60 grayscale border-slate-200 bg-slate-50 shadow-none' : 'shadow-sm'}`}>
                                    <button onClick={(e) => { e.stopPropagation(); toggleTaskStatus(t); }} className={`w-5 h-5 rounded-full border flex items-center justify-center shrink-0 transition-all ${t.status === TaskStatus.DONE ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-white border-slate-200 hover:border-orange-400'}`}>
                                       {t.status === TaskStatus.DONE && <i className="fa-solid fa-check text-[8px]"></i>}
                                    </button>
                                    <div className="flex-1 min-w-0">
                                       <span className={`text-[12px] md:text-[13px] font-bold truncate block ${t.status === TaskStatus.DONE ? 'line-through opacity-30' : 'text-slate-700'}`}>{t.title}</span>
            ...
            <div className="max-w-3xl mx-auto py-4 md:py-6 space-y-3 md:space-y-4 px-3 md:px-4">
              <div className="flex items-center justify-between px-2 mb-1">
                 <Typography variant="tiny" className="text-slate-400 font-black uppercase tracking-widest text-[7px] md:text-[8px]">Стратегічні цілі циклу</Typography>
                 <Badge variant="orange" icon="fa-mountain-sun" className="text-[6px] md:text-[7px]">{filteredGoals.length} ЦІЛЕЙ</Badge>
              </div>
              {filteredGoals.map(goal => (
              ...
              {filteredGoals.length === 0 && (
                <div className="py-16 md:py-20 text-center opacity-20 flex flex-col items-center">
                   <i className="fa-solid fa-mountain-sun text-4xl md:text-5xl mb-4"></i>
                   <Typography variant="h2" className="text-base md:text-lg font-black uppercase">Мапа стратегії порожня</Typography>
                   <button onClick={() => setModalState({ open: true })} className="mt-4 text-orange-600 font-black uppercase text-[8px] md:text-[9px] hover:underline decoration-2 underline-offset-4">+ Встановити ціль</button>
                </div>
              )}
            </div>
          )}
        ...
        {/* Mobile Quick Add Goal Button */}
        {isMobile && !modalState.open && activeMode !== 'structure' && (
          <button 
            onClick={() => setModalState({ open: true })}
            className="fixed bottom-24 right-6 w-14 h-14 rounded-2xl bg-orange-600 text-white shadow-2xl shadow-orange-200 flex items-center justify-center z-40 animate-in zoom-in"
          >
      ...
      {/* Details Side Panel - Integrated for Mobile */}
      <div className={`flex h-full border-l border-slate-100 z-[110] bg-white shrink-0 transition-all duration-300 ${isMobile ? (selectedTaskId || selectedSubProjectId || selectedHabit ? 'fixed inset-0 w-full translate-x-0' : 'fixed inset-0 w-full translate-x-full') : ''}`}>
        {!isMobile && (
        ...
        <div style={{ width: isMobile ? '100vw' : detailsWidth }} className="h-full bg-white relative flex flex-col overflow-hidden">
          {selectedTaskId ? (
